...
if (formContact) {
  formContact.addEventListener('submit', function (e) {
    ...
    var el = e.target;
    var errors = [];
    var divErrors = document.querySelector('.error-messages');
    var firstName = formContact.querySelector('#first-name').value;
    var email = formContact.querySelector('#email').value;
    var telefone = formContact.querySelector('#telefone').value;
    ...
    if (errors.length > 0) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          divErrors.innerHTML += "".concat(erro, "<br>");
        }
      } catch (err) {
        _iterator.e(err);
      } finally {
